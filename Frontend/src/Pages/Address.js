...
function Address() {
  ...
  const [getSecPhone, setSecPhone] = useState(null);
  ...
  async function submitFormHandler(event) {
    ...
    if (!event.target.secoundaryPhone.value) {
      setSecPhone(null);
    } else {
      setSecPhone(event.target.secoundaryPhone.value);
    }
    if (!event.target.street.value) {
      setStreet(null);
    } else {
      setStreet(event.target.street.value);
    }

    var jsonData = {
      name: event.target.name.value,
      phone: event.target.phone.value,
      secoundaryPhone: getSecPhone,
